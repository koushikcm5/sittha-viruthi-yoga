...
export default function AdminDashboard({ navigation }) {
  ...
  const loadAppointments = async () => {
    try {
      console.log('Loading appointments from:', `${API_URL}/appointments/admin/all`);
      ...
      if (res.ok) {
        ...
        if (text) {
          ...
          console.log('Parsed appointments:', data);
  ...
  const loadQA = async () => {
    try {
      const res = await fetch(`${API_URL}/qa/admin/all`);
      if (res.ok) {
  ...
  const answerQuestion = async (id) => {
    ...
    try {
      await fetch(`${API_URL}/qa/admin/answer/${id}`, {
        method: 'PUT',
  ...
  const approveAppointment = async (id) => {
    ...
    try {
      await fetch(`${API_URL}/appointments/admin/approve/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const rejectAppointment = async (id) => {
    try {
      await fetch(`${API_URL}/appointments/admin/reject/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const loadUserDetails = async (user) => {
    try {
      const res = await fetch(`${API_URL}/attendance/user/${user.username}`);
      const data = await res.json();
  ...
  const renderAppointments = () => (
    <View style={styles.card}>
      ...
      {Array.isArray(appointments) && appointments.length > 0 ? (
        ...
        <Text style={styles.noData}>No appointment requests yet</Text>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  filterLabel: { fontSize: 14, fontWeight: '700', color: '#6B7280', marginBottom: 12 },
  filterOptions: { flexDirection: 'row', gap: 12, marginBottom: 24 },
  filterOption: { flex: 1, padding: 12, backgroundColor: '#F3F4F6', borderRadius: 12, alignItems: 'center', borderWidth: 2, borderColor: 'transparent' },
  filterOptionActive: { backgroundColor: '#DBEAFE', borderColor: '#1B3B6F' },
  filterOptionText: { fontSize: 14, fontWeight: '600', color: '#6B7280' },
  filterOptionTextActive: { color: '#1B3B6F', fontWeight: '800' },
  filterActions: { flexDirection: 'row', gap: 12 },
  clearFilterBtn: { flex: 1, padding: 16, backgroundColor: '#F3F4F6', borderRadius: 12 },
  clearFilterText: { fontSize: 15, fontWeight: '700', color: '#6B7280', textAlign: 'center' },
  applyFilterBtn: { flex: 1, padding: 16, backgroundColor: '#1B3B6F', borderRadius: 12 },
  applyFilterText: { fontSize: 15, fontWeight: '800', color: '#FFFFFF', textAlign: 'center' },
  ...
  deleteModalTitle: { fontSize: 22, fontWeight: '900', color: '#111827', marginBottom: 12, textAlign: 'center' },
  deleteModalDesc: { fontSize: 15, color: '#6B7280', marginBottom: 24, textAlign: 'center', lineHeight: 22 },
  deleteModalActions: { flexDirection: 'row', gap: 12 },
  cancelDeleteBtn: { flex: 1, padding: 14, backgroundColor: '#F3F4F6', borderRadius: 12 },
  cancelDeleteText: { fontSize: 15, fontWeight: '700', color: '#6B7280', textAlign: 'center' },
  ...
  attendanceStatus: { fontSize: 12, color: '#6B7280', marginTop: 4 },
  ...
  statusBtnTextActive: { color: '#3B82F6', fontWeight: '800' },
  statusBtnTextActiveGreen: { color: '#27AE60', fontWeight: '800' },
  statusBtnTextActiveRed: { color: '#E74C3C', fontWeight: '800' },
  ...
  calendarDate: { fontSize: 15, fontWeight: '700', color: '#27AE60', marginBottom: 4 },
  calendarDateAbsent: { color: '#E74C3C' },
  calendarStatus: { fontSize: 13, fontWeight: '600' },
  ...
  appointmentBadgeText: { color: '#FFF', fontSize: 9, fontWeight: '800', letterSpacing: 0.2 },
  appointmentReason: { fontSize: 12, color: '#1B3B6F', marginBottom: 5, lineHeight: 16 },
  appointmentDate: { fontSize: 10, color: '#6B7280', marginBottom: 2, fontWeight: '500' },
  appointmentScheduled: { fontSize: 10, color: '#10B981', fontWeight: '700', marginBottom: 2 },
  appointmentNotes: { fontSize: 10, color: '#6B7280', fontStyle: 'italic', marginBottom: 5, lineHeight: 14 },
  ...
  userDetailLabel: { fontSize: 14, fontWeight: '600', color: '#6B7280' },
  userDetailValue: { fontSize: 14, fontWeight: '700', color: '#1B3B6F' },
  ...
  attendanceStatValue: { fontSize: 24, fontWeight: '800', color: '#1B3B6F', marginBottom: 4 },
  attendanceStatLabel: { fontSize: 11, color: '#6B7280', fontWeight: '600' },
  attendanceCalendarTitle: { fontSize: 16, fontWeight: '700', color: '#1B3B6F', marginBottom: 12 },
