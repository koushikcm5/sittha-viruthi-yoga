...
export default function AdminContentManager({ navigation, route }) {
  ...
  const getVideoForLevel = (level) => {
    return videos.find(v => v.level === level);
  };
  ...
  const renderVideoTab = () => {
    ...
    <View style={styles.formContainer}>
      ...
      <Text style={styles.infoText}>Only one video per level. {existingVideo ? 'Update existing video below.' : 'Add a new video.'}</Text>
      ...
      {videoForm.level === '3' && (
        <>
          ...
          <View style={styles.pickerWrapper}>
            <MaterialIcons name="filter-1" size={18} color="#6B7280" style={styles.pickerIcon} />
            <View style={styles.pickerContainer}>
              <Picker
                selectedValue={videoForm.part}
                onValueChange={(value) => setVideoForm({...videoForm, part: value})}
                style={styles.picker}>
                <Picker.Item label="Part 1" value="1" />
                <Picker.Item label="Part 2" value="2" />
              </Picker>
            </View>
          </View>
        </>
      ...
      <TouchableOpacity style={styles.addBtn} onPress={updateVideo}>
